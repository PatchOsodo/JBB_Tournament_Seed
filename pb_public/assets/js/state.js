...
const UI = {
  ...
  async openModal(fixture, isEdit = false) {
    ...
    const timeEl  = document.getElementById('modal-scheduled-time');
    if (timeEl) timeEl.value = fixture.scheduled_time || '';

    // Court list is fetched fresh each time the modal opens, so whoever's
    // scoring the match always sees the current real court list — this is
    // the "assign court at the moment of scoring" flow, not a pre-planned
    // schedule that might be stale by matchday.
    const courtEl = document.getElementById('modal-court');
    if (courtEl) {
      courtEl.innerHTML = '<option value="">Loading courts…</option>';
      try {
        const courts = await DB.getCourts(fixture.tournament);
        const options = ['<option value="">— Not assigned yet —</option>']
          .concat(courts.map(c => `<option value="${c.id}" ${fixture.court === c.id ? 'selected' : ''}>${escHtml(c.court_name)}</option>`));
        courtEl.innerHTML = options.join('');
      } catch (e) {
        Logger.warn('openModal court fetch failed', { error: e.message });
        courtEl.innerHTML = '<option value="">— Not assigned yet —</option>';
      }
    }
