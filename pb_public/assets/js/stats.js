...
function renderAuthBar() {
  ...
  if (navUsers) navUsers.style.display = (user?.role === 'super_admin' || user?.role === 'tournament_admin') ? '' : 'none';
  const navCourts = document.getElementById('nav-courts');
  if (navCourts) navCourts.style.display = (user?.role === 'super_admin' || user?.role === 'tournament_admin') ? '' : 'none';
  if (!ctrl) return;
  if (user) {
    const roleLabel = {
